...
export default function EditableJobDescription(props: EditableJobDescriptionProps) {
    ...
    return (
        <Box height="84vh" overflowY="scroll" p={5}>
            {showApplicants ? 
            <Applicants onBack={showApplicantsHandler} applicants={props.job.appliedBy}/> : 
            <>
                ...
                <HStack mt={3} onMouseEnter={onMouseEnterHandler} onMouseLeave={onMouseLeaveHandler} _hover={{cursor: "pointer"}} onClick={showApplicantsHandler}>
                    <Icon as={MdPeopleAlt} boxSize={6} color="#666666" mr={1}/>
                    <Text 
                    fontSize={14}
                    as={mouseHover ? 'u' : undefined}
                    >
                        {props.job.appliedBy.length} {props.job.appliedBy.length == 1 ? "applicant" : "applicants"}
                    </Text>
                </HStack>
