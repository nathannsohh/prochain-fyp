...
interface OrgProfileHeadPreviewProps {
...
export default function OrgProfileHeadPreview(props: OrgProfileHeadPreviewProps) {
    ...
    return (
        ...
        position="relative">
            <Center cursor="pointer" onMouseEnter={() => setProfilePicHover(true)} onMouseLeave={() => setProfilePicHover(false)}>
                ...
                zIndex="2"
                mt="300px">
                    <Image src={profileImageChanged && profileImageSrc ? profileImageSrc! : `http://127.0.0.1:8080/ipfs/${props.orgData?.profile_picture_hash}`} alt="profile picture" layout="fill" objectFit="cover"/>
                </Box>
            </Center>
            ...
            <Box height="50%" position="relative">
                ...
                />
                <Image src={profileBannerChanged && profileBannerSrc ? profileBannerSrc! : `http://127.0.0.1:8080/ipfs/${props.orgData?.profile_banner_hash}`} alt="profile banner" layout="fill" objectFit="cover"/>
            </Box>
            <VStack p={5} mt={2} width="100%" height="50%">
                <Text fontSize="23px" fontWeight="semibold" position="absolute">{props.orgData?.company_name}</Text>
                <HStack width="100%" mb={1} alignItems="center">
                <Box bg="#EEEEEE" p={1} borderRadius="10px" pl="16px" pr="16px">
                    <Text fontSize="13px" color="#555353"><b>{props.followers?.toString()}</b> Connections</Text>
                </Box>
                <Spacer />
                </HStack>
                <Box bg="#C6EAFF" p={1} borderRadius="20px" pl="16px" pr="16px">
                    <Text fontSize="11px" fontWeight="bold" color="#818181">{formatAddress(props.orgData?.wallet_address)}</Text>
                </Box>
                <Text fontSize="13px">{props.orgData?.industry}</Text>
                <Text fontWeight="700" color="#5F5F5F" fontSize="12px">{props.orgData?.location}</Text>
                <Spacer />
            </VStack>
        </Box>
    )
